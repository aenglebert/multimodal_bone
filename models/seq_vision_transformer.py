...
__all__ = ['SeqVisionTransformer']  # model_registry will add each entrypoint fn to this
...
class Block(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.interaction_block is not None:
            output_dict = self.interaction_block(output_dict)
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        cls_tokens = cls_tokens + self.attn(self.norm1(cls_tokens))
...
class SeqVisionTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        # Interaction Block (shared across all layers)
    ...
    def forward_features(self, x, seq_len=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.blocks({"x": x, "seq_len": seq_len})["x"]
        x = self.norm(x)
    ...
    def forward(self, x):
        ...
        # if x is a 5D tensor with B,S,C,H,W layout, with S as seq_len, reshape to B*S,C,H,W
        if x.dim() == 5:
            bs = x.shape[0]
            seq_len = x.shape[1]
        ...
        x = self.forward_features(x, seq_len=seq_len)
        x = self.forward_head(x)

        # reshape to B,S,n_classes
...
@torch.no_grad()
def _load_weights(model: SeqVisionTransformer, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    return build_model_with_cfg(
        SeqVisionTransformer, variant, pretrained,
